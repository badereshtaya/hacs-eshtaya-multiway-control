...
const SMART_GROUP_DOMAINS = ["binary_sensor","button","cover","event","fan","light","lock","media_player","notify","sensor","switch","valve"];
const SMART_PHYSICAL_DOMAINS = new Set(["button","cover","fan","light","lock","media_player","switch","valve"]);
...
class EshtayaMultiWayPanel extends HTMLElement {
    ...
    try {
      ...
      this._entities = entities
        ...
        .map(e => {
          ...
          return {...e,_domain:(e.entity_id||"").split(".")[0],_device_class:attrs.device_class||"",_unit:attrs.unit_of_measurement||"",_state_class:attrs.state_class||""};
        })
  ...
  _smartGroupCard(g){
    ...
    return `<article class="group-card ${g.enabled?'':'disabled-card'}">
      <div class="card-head"><div class="group-title"><div class="group-icon"><ha-icon icon="${meta.icon}"></ha-icon></div><div><h3>${this.esc(g.name)} ${g.favorite?'★':''}</h3><small>${this.esc(meta.label)} · ${g.kind==='physical'?this.t('physical'):this.t('virtual')} · ${g.members.length} ${this.t('members')}</small></div></div><span class="badge health-${this.esc(r.health||'healthy')}"><span></span>${this._smartHealthLabel(r.health)}</span></div>
      ...
      <div class="card-actions"><button class="${g.enabled===false?'primary':''}" data-action="smart-toggle-enabled" data-id="${g.id}" data-enabled="${g.enabled!==false}"><ha-icon icon="mdi:${g.enabled===false?'play-circle-outline':'pause-circle-outline'}"></ha-icon>${g.enabled===false?this.t('enable'):this.t('disable')}</button>${this._smartQuickActions(g)}<button data-action="smart-sync" data-id="${g.id}" ${g.enabled===false?'disabled':''}><ha-icon icon="mdi:sync"></ha-icon>${this.t('sync')}</button><button data-action="smart-test" data-id="${g.id}" ${g.enabled===false?'disabled':''}><ha-icon icon="mdi:test-tube"></ha-icon>${this.t('test')}</button><button data-action="smart-edit" data-id="${g.id}"><ha-icon icon="mdi:pencil"></ha-icon>${this.t('edit')}</button><button data-action="smart-clone" data-id="${g.id}"><ha-icon icon="mdi:content-copy"></ha-icon>${this.t('clone')}</button><button data-action="smart-template" data-id="${g.id}"><ha-icon icon="mdi:content-save-outline"></ha-icon>${this.t('template')}</button><button class="danger-link" data-action="smart-delete" data-id="${g.id}"><ha-icon icon="mdi:delete-outline"></ha-icon>${this.t('del')}</button></div>
    </article>`;
  }
  ...
  _renderSmartEditorDialog(){
    ...
    const memberCandidates=this._smartMemberCandidates(type,g.members||[],compatibility);
    const onOff=SMART_ON_OFF_DOMAINS.has(type), statePolicy=['light','switch','binary_sensor'].includes(type), sensorType=type==='sensor';
    ...
    return `<dialog id="smart-editor" class="modal"><form id="smart-form"><div class="modal-head"><div><h2>${g.id?this.t('edit'):this.t('addSmart')}</h2><p>${this.t('draftSafe')}</p></div><button type="button" class="icon-btn" data-action="smart-close"><ha-icon icon="mdi:close"></ha-icon></button></div><div class="modal-body"><div class="draft-safe"><ha-icon icon="mdi:shield-check"></ha-icon>${this.t('draftSafe')}</div>
      <div class="form-grid two"><label class="field"><span>${this.t('groupName')}</span><input name="name" value="${this.esc(g.name||'')}" required></label><label class="field"><span>${this.t('groupDomain')}</span><select name="group_type">${this._smartTypeOptions(type)}</select></label><label class="field"><span>${this.t('kind')}</span><select name="kind"><option value="virtual" ${!physical?'selected':''}>${this.t('virtual')}</option><option value="physical" ${physical?'selected':''} ${physicalSupported?'':'disabled'}>${this.t('physical')}</option></select><small>${physicalSupported?this.t('richNativeHint'):(['binary_sensor','event','sensor'].includes(type)?this.t('readOnly'):this.t('virtualOnly'))}</small></label><label class="field"><span>${this.t('area')}</span><select name="area_id"><option value="">${this.t('none')}</option>${this._areas.map(a=>`<option value="${a.area_id}" ${g.area_id===a.area_id?'selected':''}>${this.esc(a.name)}</option>`).join('')}</select></label></div>
      <div class="native-domain-banner"><ha-icon icon="${typeMeta.icon}"></ha-icon><div><b>${this.esc(typeMeta.label)}</b><small>${this.t('richNativeHint')}</small></div></div>
      ...
      <div class="controllers-editor"><div class="section-head"><div><h3>${this.t('members')}</h3><p>${this.t('compatibilityHint')}</p></div><button type="button" data-action="smart-member-add"><ha-icon icon="mdi:plus"></ha-icon>${this.t('addMember')}</button></div>${(g.members||[]).map((m,i)=>this._smartMemberRow(m,i,type)).join('')}${this._entityDatalistItems('smart-member-list',memberCandidates)}</div>
  ...
  _bind() {
    ...
    if (smartForm) {
      ...
      smartForm.addEventListener("change", e=>{
        ...
        if(name==="kind"||name==="compatibility_mode"||memberField==="entity_id")this._render();
      });
    }
