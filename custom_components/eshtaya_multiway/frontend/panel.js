...
const I18N = {
  en: {
    ...
    testHint: "Each action below is isolated: it tests that device only and will not intentionally propagate through the group.",
  ...
  ar: {
    ...
    testHint: "كل زر بالأسفل يُختبر بشكل منفصل؛ الاختبار لا يفترض أن ينشر الأمر على باقي المجموعة.",
...
class EshtayaMultiWayPanel extends HTMLElement {
  ...
  async _refresh(includeActivity = false) {
    ...
    try {
      ...
      if (!this._loading && !this._editing && !this._testResult && !this._settingsDirty) this._render();
    } catch (err) {
  ...
  _render() {
    ...
    this.shadowRoot.innerHTML = `
      ...
      <main class="app" dir="${rtl ? "rtl" : "ltr"}">
        <header class="hero">
          <div>
            <div class="eyebrow">ESHTAYA SMART · v${this.esc(this._data.version || "2.1.0")}</div>
            <h1>${this.t("title")}</h1>
            <p>${this.t("subtitle")}</p>
          </div>
  ...
  _renderEditorDialog() {
    ...
    const behavior = { debounce_ms:120, performance_mode:"instant", auto_heal:true, output_restore_policy:"adopt", command_timeout:null, max_retries:null, ...(g.behavior||{}) };
    return `<dialog id="editor" class="modal"><form id="group-form" method="dialog">
      ...
      <div class="modal-body">
        <div class="draft-safe"><ha-icon icon="mdi:content-save-check-outline"></ha-icon><span>${this.t("draftSafe")}</span></div>
        ...
        <details class="advanced" ${this._advancedOpen?"open":""}><summary><ha-icon icon="mdi:tune-variant"></ha-icon>${this.t("behavior")}</summary><div class="advanced-body">
  ...
  _renderTestDialog() {
    ...
    return `<dialog id="test-dialog" class="modal test-modal"><div class="modal-head"><div><h2>${this.t("testTitle")}</h2><p>${this.esc(r.name)}</p></div><button type="button" class="icon-btn" data-action="close-test"><ha-icon icon="mdi:close"></ha-icon></button></div><div class="modal-body"><div class="test-intro"><ha-icon icon="mdi:shield-check-outline"></ha-icon><div><b>${this.t("testTitle")}</b><span>${this.t("testHint")}</span></div><div class="test-health badge health-${this.esc(r.health)}">${this._healthLabel(r.health)}</div></div><div class="test-list">${r.entities.map(e=>{const busy=this._testBusy===e.entity_id;const label=e.test_action==="press"?this.t("press"):this.t("toggle");const last=this._testResults[e.entity_id];return `<div class="test-row"><span class="dot ${this._stateTone(e.state)}"></span><div class="test-entity"><code>${this.esc(e.entity_id)}</code><small>${this.esc(e.role)} · ${this.esc(e.domain||"")} · ${e.commandable?"testable":"read-only"}${last?` · <b class="${last.ok?"test-ok":"test-fail"}">${last.latency_ms} ms</b>`:""}</small></div><span class="state-chip state-${this.esc(e.state)}">${this.esc(e.state)}</span>${e.test_action?`<button type="button" class="secondary test-action" data-action="test-entity" data-id="${this.esc(r.group_id)}" data-entity="${this.esc(e.entity_id)}" ${busy?"disabled":""}><ha-icon icon="mdi:${e.test_action==="press"?"gesture-tap-button":"toggle-switch"}"></ha-icon>${busy?this.t("testing"):label}</button>`:`<button type="button" class="secondary test-action" disabled>—</button>`}</div>`}).join("")}</div></div><div class="modal-actions test-actions"><button class="secondary" type="button" data-action="test-resync" data-id="${this.esc(r.group_id)}"><ha-icon icon="mdi:sync"></ha-icon>${this.t("resync")}</button><button class="primary" type="button" data-action="close-test">${this.t("close")}</button></div></dialog>`;
  }
  ...
  async _action(btn) {
    ...
    try {
      ...
      if (action === "close-editor") { this._editing=null; this._advancedOpen=false; this._editorScrollTop=0; return this._render(); }
      if (action === "close-test") {
        const gid=this._testResult?.group_id; this._testBusy=null;
        if(gid){try{await this._hass.callWS({type:`${DOMAIN}/sync`,group_id:gid});}catch(_){}}
        this._testResult=null; this._testResults={}; await this._refresh(false); return this._render();
      }
  ...
  _openEditor(group) {
    this._advancedOpen=false; this._editorScrollTop=0;
    this._editing = group ? JSON.parse(JSON.stringify(group)) : {
      ...
      behavior:{debounce_ms:120,performance_mode:"instant",auto_heal:true,output_restore_policy:"adopt",command_timeout:null,max_retries:null}
    };
    this._render();
  }
  ...
  _captureDraft() {
    ...
    this._editing.behavior={debounce_ms:Number(fd.get("debounce_ms")||120),performance_mode:fd.get("performance_mode")||"instant",auto_heal:fd.get("auto_heal")==="on",output_restore_policy:fd.get("output_restore_policy")||"adopt",command_timeout:fd.get("command_timeout")===""?null:Number(fd.get("command_timeout")),max_retries:fd.get("max_retries")===""?null:Number(fd.get("max_retries"))};
