...
const EXTRA_I18N = {
  en: {
    ...
    nativeGroups:"Home Assistant groups", importAsSmart:"Import as Smart Group", originalSafe:"Original Home Assistant group stays unchanged.", areaSetup:"Area commissioning", areaHint:"Prioritize entities by Area and create groups quickly.", groupArea:"Create virtual group from area",
  ...
  ar: {
    ...
    nativeGroups:"جروبات Home Assistant الموجودة", importAsSmart:"استيراد كـ Smart Group", originalSafe:"الجروب الأصلي في Home Assistant يبقى بدون تعديل.", areaSetup:"تجهيز حسب المنطقة", areaHint:"رتّب الأجهزة حسب Area وأنشئ الجروبات بسرعة.", groupArea:"إنشاء جروب وهمي من المنطقة",
...
class EshtayaMultiWayPanel extends HTMLElement {
    ...
    this.shadowRoot.innerHTML = `
      ...
      <main class="app" dir="${rtl ? "rtl" : "ltr"}">
        <header class="hero control-hero">
          <div><div class="eyebrow">ESHTAYA SMART · v${this.esc(this._data.version || "3.0.0")}</div><h1>${this.t("controlCenter")}</h1><p>${this.t("subtitle")}</p></div>
  ...
  _renderDashboard() {
    ...
    return `<div class="dashboard-grid">
      <section class="dash-main settings-card">
        <div class="section-head"><div><h2>${this.t("controlCenter")}</h2><p>${this._smart.settings?.project_name?this.esc(this._smart.settings.project_name):"Eshtaya Smart"}</p></div><span class="version-chip">v${this.esc(this._data.version||"3.0.0")}</span></div>
  ...
  _renderSmartGroups(){
    ...
    return `<div class="toolbar"><label class="search"><ha-icon icon="mdi:magnify"></ha-icon><input id="smart-search" value="${this.esc(this._smartSearch)}" placeholder="${this.t("search")}"></label><button class="primary" data-action="smart-add"><ha-icon icon="mdi:plus"></ha-icon>${this.t("addSmart")}</button></div>
      ${groups.length?`<div class="group-grid">${groups.map(g=>this._smartGroupCard(g)).join("")}</div>`:`<div class="empty"><ha-icon icon="mdi:lightbulb-group-outline"></ha-icon><h2>${this.t("smartEmpty")}</h2><p>${this.t("smartEmptyHint")}</p><button class="primary" data-action="smart-add">${this.t("addSmart")}</button></div>`}`;
  }
  ...
  _smartGroupCard(g){
    ...
    return `<article class="group-card ${g.enabled?'':'disabled-card'}">
      ...
      <div class="meta"><span><ha-icon icon="mdi:source-branch"></ha-icon>${this.t('lastSource')}: <b>${this.esc(r.last_source||'—')}</b></span><span>${this.t('response')}: <b>${this._responseLabel(r.response_class)}</b></span></div>
      <div class="card-actions"><button data-action="smart-state" data-id="${g.id}" data-state="on"><ha-icon icon="mdi:power"></ha-icon>ON</button><button data-action="smart-state" data-id="${g.id}" data-state="off"><ha-icon icon="mdi:power-off"></ha-icon>OFF</button><button data-action="smart-sync" data-id="${g.id}"><ha-icon icon="mdi:sync"></ha-icon>${this.t('sync')}</button><button data-action="smart-test" data-id="${g.id}"><ha-icon icon="mdi:test-tube"></ha-icon>${this.t('test')}</button><button data-action="smart-edit" data-id="${g.id}"><ha-icon icon="mdi:pencil"></ha-icon>${this.t('edit')}</button><button data-action="smart-clone" data-id="${g.id}"><ha-icon icon="mdi:content-copy"></ha-icon>${this.t('clone')}</button><button data-action="smart-template" data-id="${g.id}"><ha-icon icon="mdi:content-save-outline"></ha-icon>${this.t('template')}</button><button class="danger-link" data-action="smart-delete" data-id="${g.id}"><ha-icon icon="mdi:delete-outline"></ha-icon>${this.t('del')}</button></div>
    </article>`;
  }
  ...
  _renderCommissioning(){
    ...
    return `<div class="panel-section commissioning-grid">
      ...
      <section class="settings-card"><div class="section-head"><div><h2>${this.t('nativeGroups')}</h2><p>${this.t('originalSafe')}</p></div></div>${this._nativeGroups.length?`<div class="native-list">${this._nativeGroups.map(g=>`<div class="native-row"><div><b>${this.esc(g.name)}</b><code>${this.esc(g.entity_id)}</code><small>${g.members.length} ${this.t('members')}</small></div><button data-action="native-import" data-entity="${this.esc(g.entity_id)}">${this.t('importGroup')}</button></div>`).join('')}</div>`:`<div class="muted-pad">${this.t('noGroups')}</div>`}</section>
  ...
  async _action(btn) {
    ...
    try {
      ...
      if (action === "native-import") { await this._hass.callWS({type:`${DOMAIN}/smart/import_ha_group`,entity_id:btn.dataset.entity});this._toast(this.t("saved"));await this._loadNativeGroups();return await this._refresh(false); }
  ...
  _captureSmartDraft() {
    ...
    g.favorite=fd.get("favorite")==="on"; g.maintenance=fd.get("maintenance")==="on"; g.locked=fd.get("locked")==="on";
